...
claude -p "Explore the codebase and evolve the app. You don't have to improve what's already there — feel free to take it in any direction, add something completely new, or even radically reimagine what the app is. The app can slowly (or suddenly) become something totally different over time. Just keep each change to one coherent idea so nothing breaks. All changes should work on both mobile and desktop.${TELEMETRY_PROMPT}
